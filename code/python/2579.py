...
num = int(rl())
stairs = [int(rl()) for _ in range(num)]
dp_table = []

dp_table.append(stairs[0])
if num == 1:
  print(dp_table.pop())
  exit(0)
dp_table.append(stairs[0] + stairs[1])
if num == 2:
  print(dp_table.pop())
  exit(0)
dp_table.append(max(stairs[0] + stairs[2], stairs[1] + stairs[2]))
if num == 3:
  print(dp_table.pop())
  exit(0)

for i in range(3, num):
  dp_table.append(max(stairs[i] + dp_table[i-2] , stairs[i] + stairs[i-1] + dp_table[i-3]))
